...
public class TrackingWebserverApplicationTests {
	...
	public void imgRouteShouldReturnOKAndPNG() throws IOException {
		...
		final WebTestClient client = WebTestClient.bindToRouterFunction(application.getImgRoute()).build();
		...
		client.get()
			...
			.expectBody(byte[].class).isEqualTo(gif);
	}
	...
	public void pingRouteShouldReturnOKWhenFileIsPresent() throws IOException {
		...
		final WebTestClient client = WebTestClient.bindToRouterFunction(application.getPingRoute()).build();
